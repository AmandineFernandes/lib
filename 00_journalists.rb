...
def short_handle(handle)
    court = handle.sort {|l, r| l.length <=> r.length}
    puts "#{court[0]}"
end

#Combien y-a-t'il de handle contenant 5 caractères
def caractere(handle)
    five = handle.count {|element| element.length == 5}
    puts "#{five} éléments qui contiennent 5 caractères"
end

#Combien commencent par une majuscule (première lettre juste après le @) ?
def capital(handle)
    majuscule = handle.count {|maj| maj[1][/[A-Z]/]}
    puts "#{majuscule} handle qui commencent par une majuscule"
end

#Trie la liste de handle par ordre alphabétique.
def trie(handle)
    short_handle = handle.sort
    print short_handle
end

#Quelle est la position dans l'array de la personne @epenser ?
def select_epenser(handle)
    personne = handle.index("@epenser")
    puts "Epenser se trouve a la position #{personne}"
end 
